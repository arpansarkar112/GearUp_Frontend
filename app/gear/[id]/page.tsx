...
if (!gear) {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-background space-y-4">
      <h1 className="text-2xl font-bold text-foreground">Gear Not Found</h1>
      <Button asChild>
      ...
      <main className="max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8 mt-2">
        <div className="mb-6">
          <Button 
            ...
            size="icon" 
            asChild
            className="rounded-full w-10 h-10 bg-card border-border text-muted-foreground hover:text-orange-500 hover:border-orange-500/20 hover:bg-orange-500/10 shadow-sm transition-all"
            title="Back to Gears"
        ...
        <div className="grid lg:grid-cols-2 gap-10">
          ...
          <div className="space-y-4">
            <div className="aspect-square bg-muted rounded-3xl overflow-hidden shadow-md relative border border-border">
              <Image 
          ...
          <div className="flex flex-col space-y-8">
            <div>
              ...
              <p className="text-lg font-medium text-muted-foreground mb-2">by {gear.brand || "GearUp"}</p>
              
              <div className="flex items-center gap-2 mb-6">
                <div className="flex items-center gap-1 text-orange-500">
                  <Star className="h-5 w-5 fill-orange-500" />
                  <span className="font-bold text-foreground text-lg">{averageRating > 0 ? averageRating.toFixed(1) : "New"}</span>
                </div>
                {reviews.length > 0 && (
                  <>
                    ...
                    }} className="text-sm font-medium text-muted-foreground hover:text-orange-500 underline underline-offset-4 transition-colors">
            ...
            {user?.role === 'PROVIDER' || user?.role === 'ADMIN' ? (
              <Card className="border-none shadow-xl bg-card overflow-hidden rounded-3xl">
                <CardContent className="p-6 sm:p-8 flex flex-col items-center justify-center space-y-4 text-center">
                  <h3 className="text-xl font-bold text-foreground mb-2">Partner View</h3>
                  <p className="text-sm text-muted-foreground">As a {user.role.toLowerCase()}, you cannot rent gear. Please log in as a customer to create reservations.</p>
                </CardContent>
              </Card>
            ) : (
              <Card className="border-none shadow-xl bg-card overflow-hidden rounded-3xl">
                <CardContent className="p-6 sm:p-8 flex flex-col items-center justify-center space-y-6">
                  <div className="text-center">
                    <h3 className="text-xl font-bold text-foreground mb-2">Ready for your adventure?</h3>
                    <p className="text-sm text-muted-foreground">Add this gear to your trip bag, and select your dates when you check out.</p>
                  </div>
        ...
        <div id="reviews" className="scroll-mt-24 mt-16 pt-16 border-t border-border">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            ...
            <div className="inline-flex items-center justify-center px-4 py-2 bg-muted rounded-full text-sm font-bold text-foreground">
              {reviews.length} {reviews.length === 1 ? 'Review' : 'Reviews'}
            </div>
          </div>

          {reviews.length === 0 ? (
            <div className="bg-card rounded-3xl p-12 text-center border-dashed border-2 border-border">
              <Star className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
              <h3 className="text-xl font-bold text-foreground">No reviews yet</h3>
              <p className="text-muted-foreground mt-2">Be the first to review this gear after your rental!</p>
            </div>
          ) : (
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {reviews.map((review) => (
                <Card key={review.id} className="overflow-hidden border-border shadow-sm transition-all hover:shadow-md bg-card">
                  <CardContent className="p-6">
                    ...
                    <div className="flex items-center gap-3 mb-4">
                       ...
                       <div>
                         ...
                         <p className="text-xs text-muted-foreground font-medium">{format(new Date(review.createdAt), "MMM d, yyyy")}</p>
                       </div>
                    </div>
